...
public class BoardService {
	...
	// 리턴 타입은 배열이나 List
	// 일부 목록을 가져올 때는 매개변수로 페이지 번호를 갖는 경우가 많다.
	...
	public board getBoard(int num) {
		// list에서 num 이라는 글번호를 가진 데이터를 찾아서 리턴
		board board = null;

		// list를 순회하면서 num과 같은 글 번호를 가진 데이터를 board에 대입
		for (board temp : list) {
			if (temp != null) {
				// 글번호를 찾으면 더 이상 반복문을 수행할 필요가 없어서 break
				if (temp.getNum() == num) {
					board = temp;
					break;
				}
	...
	// 글쓰기를 처리하는 메소드
	// 데이터 삽입을 처리하는 메소드
	// 수행 결과를 알려주고자 할 때는 boolean 이나 int로 리턴
	...
	// 리턴 타입은 삽입과 동일
	// 매개변수도 삽입과 동일
	// 수정을 할 때는 기본키(글번호)의 값이 자동으로 설정되는 것이 다르다.
	...
	// 삭제를 처리하는 메소드
	// 삭제를 할 때는 데이터 전체가 필요하지 않고 일반적으로 기본키만 필요함
